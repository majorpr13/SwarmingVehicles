...
#include "ui_parameterrequestwindow.h"

parameterRequestWindow::parameterRequestWindow(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::parameterRequestWindow)
{
    ui->setupUi(this);
}

parameterRequestWindow::~parameterRequestWindow()
{
    emit(closingWindowEmpty());
    delete ui;
}

void parameterRequestWindow::setValues(const QVector<Command_Helper::defaultCMD_Structure> &textLabels)
{
...
void parameterRequestWindow::on_pushButton_Ok_clicked()
{
    ...
    emit(closingWindowFull(returnVector));
}

void parameterRequestWindow::on_pushButton_Cancel_clicked()
{
    emit(closingWindowEmpty());
}
